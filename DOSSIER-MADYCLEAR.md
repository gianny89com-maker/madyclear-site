# DOSSIER MAÎTRE MADYCLEAR

Mise à jour : 17 septembre 2026

## Source de vérité

- Site officiel : https://www.madyclear.fr/
- Dépôt de production : `gianny89com-maker/madyclear-site`
- Branche publique : `main`
- Le domaine temporaire ChatGPT Site ne doit pas être utilisé dans Google Business Profile, les annuaires ou les supports commerciaux.

## Identité validée

- Nom commercial : MADYCLEAR
- Activité prioritaire : nettoyage professionnel de textiles
- Méthode : injection-extraction et détachage après diagnostic
- Publics : particuliers et professionnels
- Zone : Martinique entière
- Téléphone et WhatsApp : 06 96 01 70 07
- Email : contact@madyclear.fr
- Adresse personnelle : non affichée au public

## Prestations actives

- canapés ;
- matelas ;
- fauteuils ;
- tapis ;
- chaises textiles ;
- prestations textiles professionnelles.

L’automobile et les vitres sont conservées en extensions futures, mais ne constituent pas l’offre principale actuellement mise en avant.

## Tarification officielle

| Prestation | Tarif complet |
|---|---:|
| Canapé 2 places | 160 € |
| Canapé 3 places | 190 € |
| Canapé angle / panoramique | 240 € |
| Fauteuil | 80 € |
| Matelas 1 place | 120 € |
| Matelas 2 places | 160 € |
| Tapis standard | 100 € |
| Chaise textile | 40 € |
| Minimum d’intervention textile | 80 € |

### Présentation commerciale validée

1. La grille affiche uniquement les tarifs unitaires complets.
2. Le client peut composer librement un pack avec plusieurs textiles.
3. Le pack est toujours sur mesure et uniquement sur devis.
4. Aucun prix de pack, pourcentage ou montant de réduction n’est affiché.
5. Le crédit d’impôt est expliqué une seule fois dans un encadré distinct.
6. Un seul exemple indicatif peut être utilisé : 160 € facturés peuvent représenter 80 € après crédit d’impôt si toutes les conditions sont remplies.

Le crédit d’impôt éventuel n’est jamais automatique. Il dépend de l’éligibilité de la prestation, du client et du dispositif applicable. Il ne constitue pas une remise commerciale et le tarif complet demeure la référence.

## Horaires

- Lundi à samedi : 7 h–22 h
- Dimanche : 7 h–12 h
- Interventions sur rendez-vous, selon les disponibilités

## Positionnement

MADYCLEAR est un spécialiste mobile du nettoyage textile en Martinique. L’entreprise se différencie par une intervention sur place, un diagnostic préalable, l’injection-extraction, une tarification annoncée avant déplacement et un parcours de devis simple par WhatsApp.

## Parcours client

1. Réception des photos, dimensions, commune et description du besoin.
2. Diagnostic du textile et estimation.
3. Confirmation du tarif et du créneau.
4. Intervention adaptée à la fibre.
5. Contrôle du résultat et recommandations de séchage.
6. Demande d’avis Google après satisfaction du client.

## Règles de communication

- Ne pas promettre la disparition de toutes les taches.
- Préciser que le résultat dépend de la fibre, de l’état et de l’ancienneté des marques.
- Employer « traitement des odeurs » et « nettoyage en profondeur ».
- N’utiliser « désinfection », « anti-acariens » ou « élimination des allergènes » que si le produit et le protocole permettent de le prouver.
- Identifier comme illustrations les images qui ne proviennent pas d’une intervention réelle.
- Ne jamais publier l’adresse personnelle.

## Priorités SEO local

1. Associer Google Business Profile uniquement à https://www.madyclear.fr/.
2. Demander l’indexation dans Google Search Console.
3. Publier des photos réelles avant/après avec accord client.
4. Obtenir et répondre aux premiers avis Google authentiques.
5. Conserver la cohérence du nom, du téléphone, des horaires et du domaine.
