...
function patchHtml(source){
  ...
  // État réel des connexions : le CRM public est actif, mais le cockpit reste local-first.
  html=html.replace("{id:'supabase',label:'Supabase',detail:'Synchronisation multi-appareils / base centrale',next:'Créer/relier le projet Supabase et ses tables métier.'}","{id:'supabase',label:'Supabase',detail:'CRM public actif • synchro cockpit à brancher',next:'La base CRM existe déjà ; brancher uniquement la synchronisation du cockpit.'}");
  ...
  if(!html.includes('id="launch-state-v183"')){
    const marker='<article class="glass operational-focus">';
    const launch=`<article class="glass operational-focus" id="launch-state-v183">
          ...
          <p class="lead-small">Le site public capte déjà les demandes. Le cockpit reste volontairement local tant que les automatisations de lancement ne sont pas activées.</p>
          <div class="ops-list">
            ...
            <div class="ops-row"><span class="ops-dot ok"></span><div><strong>Cockpit personnel</strong><small>CRM, devis, notes, sauvegardes sur cet appareil</small></div><span class="ops-state connected">LOCAL</span></div>
